...
class SystemTest < ActionDispatch::IntegrationTest
    ...
    test "should test application" do
        ...
        # Mi iscrivo al gruppo dei samurai
        post group_memberships_path(group_uuid: @samurai.uuid)
        assert_redirected_to group_path(uuid: @samurai.uuid)
        ...
        get user_invitations_path(@peach)
        assert_response :success
        assert_difference('Invitation.count', 0) do
            assert_difference('Membership.count', 1) do
                get group_accept_invitation_path(group_uuid: @pirati.uuid, url_string: @invitation.url_string)
            end
        end
        ...
        # Invio un messaggio vuoto con un allegato
        assert_difference('Message.count') do
        ...
        assert_redirected_to group_invitations_path(group_uuid: @funghi.uuid)
        ...
        patch group_pin_message_path(group_uuid: @funghi.uuid, id: @messaggio_peach.id)
        @messaggio_peach.reload
        assert @messaggio_peach.pinned
        ...
        # Modifico l'evento e cambio il luogo
        patch group_event_path(group_uuid: @funghi.uuid, id: @event.id), params: {
            event: { place: "Castello di Bowser" }
        }
    
        assert_redirected_to group_path(uuid: @funghi.uuid)
        @event.reload
        assert_equal "Castello di Bowser", @event.place
        ...
        # Accetto l'invito
        get user_invitations_path(@luigi)
        assert_response :success
        assert_difference('Invitation.count', -1) do
            assert_difference('Membership.count', 1) do
                get group_accept_invitation_path(group_uuid: @funghi.uuid, url_string: @invito_luigi.url_string)
            end
        end

        # Invia un messaggio nel nuovo gruppo
        assert_difference('Message.count') do
            assert_difference('Attachment.count') do
              post group_messages_path(group_uuid: @funghi.uuid), params: {
                message: {
        ...
        # Accetto l'invito
        get user_invitations_path(@mario)
        assert_response :success
        assert_difference('Invitation.count', -1) do
            assert_difference('Membership.count', 1) do
                get group_accept_invitation_path(group_uuid: @funghi.uuid, url_string: @invito_mario.url_string)
            end
        end
        ...
        assert_difference("Membership.count", -1) do
            delete group_membership_path(group_uuid: @funghi.uuid, user_id: @luigi.id)
        end
        ...
        # # Dal form di reset
        # @peach_reset = assigns(:user)

        # # Seguo tutta la procedura per il reset
        # get edit_password_reset_path(@peach_reset.password_token, email: @peach.email)
        # patch password_reset_path(@peach_reset.password_token, email: @peach.email), params: { user:{
